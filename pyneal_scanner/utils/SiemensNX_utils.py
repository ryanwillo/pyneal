""" Set of classes and methods specific to Siemens scanning environments
    NX version: mosaic is deprecated
...
import nibabel.nicom.dicomwrappers as nic
import nibabel.nifti1 as nin
import zmq

# regEx for Siemens style file naming
Siemens_filePattern = re.compile('\d{3}_\d{6}_\d{6}_.+\.dcm')

# regEx for pulling the volume field out of the dicom file name
Siemens_VolumeNumberField = re.compile('(?<=\d{6}_)\d{6}')
Siemens_SeriesNumberField = re.compile('(?<=\d{3}_)\d{6}(?=_\d{6}_.+\.dcm)')
...
class Siemens_DirStructure():
    ...
    In Siemens NX environments, using the ideacmdtool, the scanner is set up to
    ...
    scanners store reconstructed images in a single DICOM volume.
    ...
    OR anatomical data will be stored in a single dicom volume, depending on
    scanner configuration. In our case, anatomical data is one 3d dicom volume.
    ...
    filename, which follows the pattern (for example):

    [session#]_[series#]_[vol#]_1.3.12.2.1107.5.2.43.66069.2023120312140892532747586.dcm
    where 66069 is the scanner serial number, and 2023... is a date/time string.
    1.3.12... is likely a software version string (todo check on this).
    ...
    def getUniqueSeries(self):
        ...
        if len(self.allMosaics) > 0:
            ...
            for f in self.allMosaics:
                seriesNums.append(Siemens_SeriesNumberField.search(f).group())
...
class Siemens_BuildNifti():
    """ Tools to build a 3D or 4D Nifti image from all of the dicom
    images in a directory.

    Input is a path to a series directory containing 3d dicom volumes.
    ...
    def __init__(self, seriesDir, seriesNum):
        ...
        # make a list of the specified raw dicom files in this dir
    ...
    def buildFunc(self, dicomFiles):
        ...
        Siemens NX scanners, each dicom file is assumed to be a 3d volume.
        This tool will construct a 4D nifti object from the individual volumes
        in the list. The 4D nifti object contains a voxel array ordered like RAS+
        as well the affine transformation to map between vox and mm space.

        Parameters
        ----------
        dicomFiles : list
            list containing the file names (file names ONLY, no path) of all
            dicom volumes to be used in constructing the final nifti
            image
        ...
        for dcm_fname in dicomFiles:
            ### Convert the dicom volume to a nifti volume
...
class Siemens_processNXdicom(Thread):
    ...
    def run(self):
        self.logger.debug('Siemens_processNXdicom started')
        ...
        while self.alive:
            ...
            if not self.dicomQ.empty():
                ...
                for m in range(numMosaicsInQueue):
                    ...
                    dcm_fname = self.dicomQ.get(True, 2)
                    ...
                    self.processDicomFile(dcm_fname)
    ...
    def processDicomFile(self, dcm_fname):
        ...
        dicomFile_root, dicomFile_name = os.path.split(dcm_fname)
...
def Siemens_launch_rtfMRI(scannerSettings, scannerDirs):
    ...
    ### Start threads to A) watch for new dicom files, and B) process
    # them as they appear
    # initialize the dicom queue to keep store newly arrived
    # dicom images, and keep track of which have been processed
    dicomQ = Queue()

    # create instance of class that will monitor sessionDir for new
    ...
    # create an instance of the class that will grab dicoms
    # from the queue, reformat the data, and pass over the socket
    # to pyneal. Start the thread going
    dicomProcessor = Siemens_processNXdicom(dicomQ, pynealSocket)
    dicomProcessor.start()
