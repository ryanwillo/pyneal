...
def Siemens_sim(inputDir, seriesNum, newSeriesNum, TR):
    ...
    print('Siemens file pattern: {}'.format(Siemens_filePattern))
    ...
    for f in glob.glob(join(inputDir, ('*_' + str(seriesNum).zfill(6) + '_*.dcm'))):
        ...
        volNum = int(os.path.split(f)[1].split('_')[2])
...
def rmFiles(seriesDir, seriesNum):
    ...
    """
    for f in glob.glob(join(seriesDir, ('???_' + str(seriesNum).zfill(6) + '_??????_*.dcm'))):
